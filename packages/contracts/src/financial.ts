...
/*
 * Staged statement rows keep their source's sign — a charge and a refund are
 * different facts, and a card's "positive is a charge" is the opposite of an
 * account's "positive is money in". Ledger records use `Amount`, which stays
 * unsigned because direction there is carried by the record's type.
 */
export const SignedAmount = z
  .string()
  .regex(/^-?\d+$/, "must be a signed decimal amount string");
...
export const CreateExpenseSettlement = z
  .object({
    ...
    /** Optional note. Bounded like `voidReason`: a sentence, not a document. */
