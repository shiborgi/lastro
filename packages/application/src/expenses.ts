...
export function createExpenseMethods(repository: ApplicationRepository) {
  return {
    ...
    async listExpenses(contextInput: unknown): Promise<Expense[]> {
      ...
      return repository.listExpenses(context.bookId);
    },
    ...
    async listExpensesPage(input: PageQuery): Promise<PageType<Expense>> {
      ...
      assertAuthorized(context, operations.listExpenses);
      return page(
        await repository.listExpenses(context.bookId),
        input.cursor,
        input.limit,
      );
    },
    ...
    async listPayments(contextInput: unknown): Promise<Payment[]> {
      const context = contextFor(contextInput);
      assertAuthorized(context, operations.listExpenses);
      return repository.listPayments(context.bookId);
    },
    ...
    async listPaymentsPage(input: PageQuery): Promise<PageType<Payment>> {
      ...
      assertAuthorized(context, operations.listExpenses);
      return page(
        await repository.listPayments(context.bookId),
        input.cursor,
        input.limit,
      );
    },
    ...
    async createExpenseSettlement(
      ...
      return repository.createExpenseSettlement(
        {
          bookId: context.bookId,
    ...
    async voidExpenseSettlement(
      ...
      return repository.voidExpenseSettlement(
        {
          bookId: context.bookId,
          id: input.id,
    ...
    async getExpenseBalance(input: ExpenseQuery): Promise<Money> {
      ...
      const expense = requireExpense(
        await repository.getExpense(context.bookId, input.id),
      );
      const total = requirePositiveMoney(expense.amount, expense.currency);
      const settlements = await repository.listExpenseSettlements(
        context.bookId,
        input.id,
      );
    ...
    async listExpenseSettlementHistory(
      ...
      assertAuthorized(context, operations.listExpenses);
      requireText(input.id, "id");
      return repository.listExpenseSettlements(context.bookId, input.id);
    },
    ...
    async listExpenseSettlementsPage(
      ...
      assertAuthorized(context, operations.listExpenses);
      return page(
        await repository.listExpenseSettlements(context.bookId),
        input.cursor,
        input.limit,
      );
    },
    ...
    async listPendingExpenses(
      ...
      const context = contextFor(contextInput);
      assertAuthorized(context, operations.listExpenses);
      return repository.listPendingExpenses(context.bookId);
    },
    ...
    async getBookPosition(input: PageQuery): Promise<{
      ...
      const context = contextFor(input.context);
      assertAuthorized(context, operations.listExpenses);
      const expenses = await repository.listPendingExpenses(context.bookId);
      const positioned = (
        await Promise.all(
