  ...
  return {
    ...
    async authenticateAgent(input: {
      ...
      const context = assertExecutionContext({
        ...
        role,
        source: input.source ?? ("MCP" satisfies Source),
    ...
    /*
     * Better Auth authenticates the human and hands us a user id; membership in
     * the requested Book is what actually decides the role, so authorization
     * stays in this package rather than leaking into the auth library.
     */
    async contextForUser(input: {
      ...
      if (!input.userId.trim() || !input.bookId.trim()) return null;
      const role = await store.getMembership(input.userId, input.bookId);
      if (!role) return null;
