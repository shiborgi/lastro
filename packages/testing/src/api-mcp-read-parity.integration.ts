...
describe("API and MCP read parity", () => {
  test("API and official MCP client return equal normalized position and pagination", async () => {
    ...
    try {
      ...
      /*
       * Asserted as coverage, not as a frozen list: pinning every name turned
       * this into a change detector that failed on any addition. What matters
       * is that the surfaces do not diverge — anything an agent needs to work
       * a Book must be here.
       */
      ...
      expect(apiPosition.totals[0].outstanding).toBe(
        BigInt(positionBody.totals[0].outstanding),
      );
