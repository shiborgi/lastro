 ...
 */
/**
 * Is this node already serving the dashboard over the tailnet?
 *
 * Checked even when `--tailscale` was not passed, because `tailscale serve` is
 * host configuration that outlives the containers: reinstalling without the
 * flag used to leave the serve running while quietly dropping its origin from
 * the trusted list, so the dashboard kept answering and every sign-in on it
 * started failing with 403. The flag says "set this up"; it should not be the
 * only way to notice it already is.
 */
async function tailscaleServesWeb(bin: string): Promise<boolean> {
  const raw = await $`${bin} serve status --json`.nothrow().quiet();
  if (raw.exitCode !== 0) return false;
  try {
    const status = JSON.parse(raw.stdout.toString()) as {
    ...
    return Object.entries(status.Web ?? {}).some(
      ([hostPort, entry]) =>
        hostPort.endsWith(`:${WEB_SERVE_PORT}`) &&
        Object.values(entry.Handlers ?? {}).some((handler) =>
          handler.Proxy?.includes(`:${WEB_PORT}`),
        ),
    );
  } catch {
    return false;
...
async function tailscaleOrigin(): Promise<string | null> {
  ...
  if (!wantsTailscale && !(await tailscaleServesWeb(bin))) return null;
...
async function install(): Promise<void> {
  ...
  /*
   * Better Auth checks the browser's Origin, and the browser is on the WEB
   * port — never the API port. Left to its default it trusts only `baseURL`,
   * which is the API, so a plain loopback install answered every dashboard
   * sign-in with 403 INVALID_ORIGIN while the same credentials worked fine
   * against the API directly. Both loopback spellings are listed because
   * whichever the operator types is the origin the proxy forwards.
   *
   * With Tailscale the origin is the tailnet name instead, and it is added
   * rather than substituted: the loopback dashboard keeps working on the host.
   */
