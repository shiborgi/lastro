...
if (noComponents) {
  ...
  console.error(
    "  THIRD_PARTY_NOTICES.md has no '## Current' section with a component list",
  );
  failed = true;
}
...
if (failed) {
  console.error(
    "THIRD_PARTY_NOTICES check failed: a copied component lacks a recorded entry",
  );
  process.exit(1);
}
